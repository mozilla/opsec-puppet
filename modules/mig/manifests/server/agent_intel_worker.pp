        ...
        'CentOS', 'RedHat', 'Ubuntu', 'Debian': {
            ...
            exec {
                'set-aiworker-db-password':
                    command     => 'sed -i "s|REPLACEMOZDEFPASSWORD|$(cat /etc/mig/worker-mozdef-password)|" /etc/mig/agent-intel-worker.cfg; rm /etc/mig/worker-mozdef-password',
                    path        => "/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin",
                    require     => [ File['/etc/mig/agent-intel-worker.cfg']];
                'set-aiworker-mq-password':
                    command     => 'sed -i "s|REPLACEMQPASSWORD|$(cat /etc/mig/worker-mq-password)|" /etc/mig/agent-intel-worker.cfg; rm /etc/mig/worker-mq-password',
                    path        => "/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin",
                    require     => [ File['/etc/mig/agent-intel-worker.cfg']];
            }
            service {
